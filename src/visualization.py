...
def create_metrics_comparison_plot(comparison_results: Dict, output_dir: str) -> str:
    ...
    bars1 = ax1.bar(x - width/2, model_a_scores, width, label='Modell A', alpha=0.8, color='skyblue')
    bars2 = ax1.bar(x + width/2, model_b_scores, width, label='Modell B', alpha=0.8, color='lightcoral')
    ...
    for i, (bar, p_val) in enumerate(zip(bars, p_values)):
        ...
        ax2.text(bar.get_x() + bar.get_width()/2., height + (0.01 if height >= 0 else -0.01),
                f'p={p_val:.3f}', ha='center', va='bottom' if height >= 0 else 'top', 
...
def create_distribution_plots(all_metrics: Dict, output_dir: str) -> str:
    ...
    available_metrics = [m for m in key_metrics if any(m in metrics for metrics in all_metrics.values())]
    ...
    for i, metric in enumerate(available_metrics):
        ...
        if model_a_data and model_b_data:
            ...
            ax.hist(model_b_data, alpha=0.7, label='Modell B', bins=10, color='lightcoral')
...
def create_all_visualizations(comparison_results: Dict, all_metrics: Dict, output_dir: str) -> List[str]:
