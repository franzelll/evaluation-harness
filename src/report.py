...
def write_markdown(out_dir: str, summary: dict, per_metric: dict):
    ...
    for metric_name, stats in per_metric.items():
        ...
        lines.append(f"- **Differenz (B - A)**: {stats['model_b_mean'] - stats['model_a_mean']:+.4f}")
        ...
        # Effektgröße
        if 'cohens_d' in stats and not (isinstance(stats['cohens_d'], float) and str(stats['cohens_d']) == 'nan'):
            cohens_d_val = stats['cohens_d']
            effect_size_desc = "klein" if abs(cohens_d_val) < 0.2 else "mittel" if abs(cohens_d_val) < 0.5 else "groß"
    ...
    lines.append("")
    lines.append("- **Signifikante Verbesserungen**: Metriken mit p < 0.05 und positiver Differenz")
    lines.append("- **Signifikante Verschlechterungen**: Metriken mit p < 0.05 und negativer Differenz")
    lines.append("- **Cohen's d**: 0.2 = kleiner Effekt, 0.5 = mittlerer Effekt, 0.8 = großer Effekt")
    lines.append("")
